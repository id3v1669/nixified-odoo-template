...
if [ "$(ls -A backup)" ]; then
    ...
    pg_restore --clean --no-acl --no-owner -j "$RESTORE_JOBS" -h "$PG_CONN_HOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" backup/*.dump 2>"$PWD/backup/pg_restore.log" || true
