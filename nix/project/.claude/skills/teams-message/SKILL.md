...
Before using the workflow, confirm that the connector tools below are available
in the current Claude Code session. If they are missing, ask the user to connect
Microsoft 365 and enable the required write permissions, then stop.

Transport is the **claude.ai Microsoft 365 connector**
(`mcp__claude_ai_Microsoft_365__teams_*`), which the user connects in their
claude.ai under **Customize → Connectors**. The project does not install a
Teams MCP server.
...
Upstream reported these results from a live connector probe on 2026-09-07.
Check the tools available in the current session; their schemas and rendering
behavior may change.
...
1. **Identity:** `get_me` must match the identity node's **Teams account must be**
   value (or **Teams auth must be** in existing memory). Preserve the saved
   name, email, and chat ID when changing the transport. Claude.ai manages
   authentication; there is no local command to re-run. If the account is
   wrong or authentication fails, ask the user to reconnect Microsoft 365
   in claude.ai under **Customize → Connectors**, then wait.
...
The connector appends each mention as a trailing `<at>` block and forces
the body to HTML. So do not write `@Name` into the text expecting it to
...
The connector cannot edit or delete a sent message. Tell the user what
happened and draft a short correction in the same chat. Wait for their
approval before sending it. If the original went to the wrong chat, say so
explicitly: those recipients still have the message.
